...
class UnionType(AnnotatedType):
    ...
    def from_annotation(
        ...
        if isinstance(annotation, ast.BinOp) and isinstance(annotation.op, ast.BitOr):
            ...
            left_type = AnnotatedType.from_annotation(annotation.left, semantic_call, enclosing_type)
            right_type = AnnotatedType.from_annotation(annotation.right, semantic_call, enclosing_type)
            ...
            if not hasattr(annotation.slice, 'elts'):
                type = AnnotatedType.from_annotation(annotation.slice, semantic_call, union_type)
                if type:
                ...
                for elt in annotation.slice.elts:
                    type = AnnotatedType.from_annotation(elt, semantic_call, union_type)
                    if type:
                        types.append(type)
                    else:
                        raise RuntimeError(f"Failed to parse union type: {ast.unparse(annotation)}")
    ...
    def __init__(self, semantic_call, types: Optional[List[AnnotatedType]]=None, enclosing_type: Optional[AnnotatedType]=None):
